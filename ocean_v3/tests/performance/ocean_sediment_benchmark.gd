extends Node3D
## PERF-SEDIMENT-V1: four-way CPU/frame-time benchmark for the persistent
## SedimentField and its two presentation layers. GPU milliseconds remain
## intentionally unavailable because this runner never synchronizes or reads
## back the GPU.
...
const CASES := [
	{"name": "A_OFF", "enabled": false, "cloud": 1.0, "wisp": 1.0},
	{"name": "B_FIELD_ONLY", "enabled": true, "cloud": 0.0, "wisp": 0.0},
	{"name": "C_FIELD_CLOUDS", "enabled": true, "cloud": 1.0, "wisp": 0.0},
	{"name": "D_FIELD_CLOUDS_WISPS", "enabled": true, "cloud": 1.0, "wisp": 1.0},
]
...
@onready var _sediment: Node = $OceanV3/OceanSedimentSystem
	...
	match _phase:
		...
		Phase.WARMING_UP:
			...
			if _phase_frame >= warmup_frames:
				...
				_dispatches_before = int(_sediment.get_debug_state().get("dispatch_count", 0))
				_phase = Phase.MEASURING
				_phase_frame = 0
				print("SEDIMENT BENCHMARK: measuring %s repetition %d/%d" % [
...
func _read_command_line_overrides() -> void:
	...
	for argument in arguments:
		if argument.begins_with("--ocean-sediment-benchmark-warmup="):
			warmup_frames = maxi(int(argument.get_slice("=", 1)), 0)
		elif argument.begins_with("--ocean-sediment-benchmark-measure="):
			measurement_frames = maxi(int(argument.get_slice("=", 1)), 1)
		elif argument.begins_with("--ocean-sediment-benchmark-stabilization="):
			stabilization_frames = maxi(int(argument.get_slice("=", 1)), 0)
		elif argument.begins_with("--ocean-sediment-benchmark-repetitions="):
			repetitions = clampi(int(argument.get_slice("=", 1)), 1, 10)


func _begin_case() -> void:
	var case: Dictionary = CASES[_case_index]
	_sediment.set("sediment_enabled", bool(case["enabled"]))
	_sediment.set("sediment_cloud_strength", float(case["cloud"]))
	_sediment.set("sediment_wisp_strength", float(case["wisp"]))
	_sediment.call("set_camera_underwater", true)
	_phase = Phase.STABILIZING
	_phase_frame = 0
	print("SEDIMENT BENCHMARK: setup %s repetition %d/%d" % [
		_current_case_name(), _repetition_index + 1, repetitions
	])
...
func _finish_measurement() -> void:
	...
	var state: Dictionary = _sediment.get_debug_state()
	var row := {
		...
		"p95_ms": p95,
		"process_ms": _cpu_process_sum_ms / float(_cpu_process_samples) if _cpu_process_samples > 0 else 0.0,
		"dispatches": int(state.get("dispatch_count", 0)) - _dispatches_before,
...
func _write_results() -> void:
	...
	var csv_path := "%s/sediment_%s.csv" % [directory, stamp]
	var txt_path := "%s/sediment_%s.txt" % [directory, stamp]
	...
	for row in _results:
		...
		txt.store_line("%s rep %d | avg %.4f ms | FPS %.2f | P95 %.4f ms | CPU %.4f ms | dispatches %d | %dx%d | %.2f Hz | clouds %d | wisps %d" % [
			row["case"], row["repetition"], row["avg_ms"], row["fps"], row["p95_ms"], row["process_ms"],
			row["dispatches"], row["resolution"], row["resolution"], row["update_hz"], row["cloud_amount"], row["wisp_amount"]
		])
	print("SEDIMENT BENCHMARK: results=%s and %s" % [csv_path, txt_path])
