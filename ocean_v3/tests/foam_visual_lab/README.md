...
Los cuatro SubViewports usan la misma geometría generada determinísticamente (16 m × 16 m, 128 celdas por lado), el mismo transform, la misma cámara virtual, Environment, luz y coordenadas world-space. El controlador sincroniza los uniforms dinámicos del material de producción, incluido `coastal_time_s`, `camera_world_xz` y el estado de breakers/reflexiones, para que las diferencias observadas pertenezcan al candidato.

En GRID, los cuatro candidatos permanecen en su `SubViewportContainer` original, renderizan continuamente y usan 640 × 360. FULLSCREEN A/B conserva `BASELINE` y el candidato seleccionado a 1920 × 1080 con `UPDATE_ALWAYS`; los otros dos quedan a 640 × 360 y desactivados. El `TextureRect` de `FullscreenHost` muestra directamente el `ViewportTexture` ya existente del candidato elegido. Alternar `SPACE` sólo cambia esa textura mostrada: no pausa, redimensiona ni reparenta ninguno de los dos candidatos A/B activos.

PERF es deliberadamente distinto: únicamente el candidato seleccionado usa `UPDATE_ALWAYS` a 1920 × 1080; los otros tres quedan a 640 × 360 y en `UPDATE_DISABLED`. Cada transición entre GRID, FULLSCREEN y PERF imprime los tamaños reales de los `SubViewport` en la consola. Al entrar en FULLSCREEN, al cambiar de candidato A/B y al volver a GRID desde un modo de alta resolución, el Lab muestra `A/B RESYNC` durante `VISUAL_RESYNC_FRAMES` (30) frames no bloqueantes. Sólo cuando el HUD muestra `A/B READY` debe hacerse una comparación temporal estricta: así se da tiempo a que los historiales de exposición y postprocesado de los viewports que estuvieron pausados o cambiaron de resolución vuelvan a avanzar.
