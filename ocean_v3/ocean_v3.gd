...
## Sunrays use a continuous procedural field in light space. Wave controls only modulate that field.
...
@export_range(0.0, 2.0, 0.01, "suffix:x") var underwater_sunrays_animation_speed := 0.12:
	set(value):
		...
		_request_visual_sync()
@export var underwater_sunrays_wave_modulation_enabled := true:
	set(value):
...
@export_range(0.0, 0.45, 0.01) var underwater_sunrays_wave_intensity_strength := 0.35:
	set(value):
		...
		_request_visual_sync()
@export_range(0.0, 0.20, 0.01) var underwater_sunrays_wave_width_strength := 0.10:
	set(value):
		underwater_sunrays_wave_width_strength = clampf(value, 0.0, 0.20)
		_request_visual_sync()
@export_range(1.0, 50.0, 0.5, "suffix: m") var underwater_sunrays_wave_depth_fade_m := 15.0:
	set(value):
...
@export var underwater_sunrays_phase_debug_constant := false:
	set(value):
		...
		_request_visual_sync()
@export_enum("CURRENT_DEPTH_DRIVEN", "ANALYTIC_SEA_PLANE") var underwater_sunrays_segment_mode := 1:
	set(value):
		underwater_sunrays_segment_mode = clampi(value, 0, 1)
		_request_visual_sync()
...
func _update_underwater_camera_state() -> void:
	...
	if camera == null:
		...
		return
	# Cheap binary guard only.  Never evaluate FFT or OceanQuery from _process().
	var sea_level := _surface_sea_level()
	_camera_water_surface_y = sea_level
	if _camera_underwater:
		if camera.global_position.y > sea_level + UNDERWATER_EXIT_MARGIN_M:
			_camera_underwater = false
	elif camera.global_position.y < sea_level - UNDERWATER_ENTER_MARGIN_M:
		_camera_underwater = true
	var half_width := maxf(underwater_transition_width_m, 0.01)
	_underwater_factor = 1.0 - smoothstep(
		sea_level - half_width,
		sea_level + half_width,
