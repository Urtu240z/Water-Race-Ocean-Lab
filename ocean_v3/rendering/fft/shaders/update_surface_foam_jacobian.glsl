...
float deperiodized_jacobian(sampler2D source_jacobian, vec2 field_world_xz) {
	float field_domain = max(params.spatial.x, 0.001);
	float phase_x = field_world_xz.x * TAU / field_domain;
	float phase_z = field_world_xz.y * TAU / field_domain;
	float blend = 0.5 + 0.5 * sin(phase_x + 0.83) * cos(2.0 * phase_z + 1.47);
	float jacobian_a = textureLod(source_jacobian, source_uv_a(field_world_xz), 0.0).r;
	float jacobian_b = textureLod(source_jacobian, source_uv_b(field_world_xz), 0.0).r;
	return mix(jacobian_a, jacobian_b, blend);
}
...
void main() {
	...
	float jacobian = deperiodized_jacobian(jacobian_map, field_world_xz);
	...
	imageStore(surface_foam_next, coord, vec4(next, source_normalized, 0.0, 1.0));
}
