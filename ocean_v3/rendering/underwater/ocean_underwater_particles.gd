...
@export_range(800, 10000, 10) var underwater_particles_fine_amount := 1000:
	set(value):
		underwater_particles_fine_amount = clampi(value, 800, 10000)
		_queue_rebuild()
@export_range(80, 10000, 10) var underwater_particles_near_amount := 140:
	set(value):
		underwater_particles_near_amount = clampi(value, 80, 10000)
		_queue_rebuild()
...
func _ready() -> void:
	# The emitter follows camera translation/orientation for new spawn positions;
	# world-space particles already emitted are not transformed with the emitter.
	top_level = true
...
func _ensure_particles() -> void:
	if _fine_particles != null and is_instance_valid(_fine_particles):
		return
	_fine_particles = get_node_or_null(^"FineParticles") as GPUParticles3D
	_near_particles = get_node_or_null(^"NearParticles") as GPUParticles3D
	if _fine_particles == null or _near_particles == null:
		push_error("OceanUnderwaterParticles: FineParticles and NearParticles must be authored scene nodes.")
		return
	_fine_process_material = _fine_particles.process_material as ShaderMaterial
	_near_process_material = _near_particles.process_material as ShaderMaterial
	var fine_quad := _fine_particles.draw_pass_1 as QuadMesh
	var near_quad := _near_particles.draw_pass_1 as QuadMesh
	_fine_render_material = fine_quad.material as ShaderMaterial if fine_quad != null else null
	_near_render_material = near_quad.material as ShaderMaterial if near_quad != null else null
	if _fine_process_material == null or _near_process_material == null \
		or _fine_render_material == null or _near_render_material == null:
		push_error("OceanUnderwaterParticles: authored particle nodes need process and render ShaderMaterials.")
		return
	_configure_particles()
...
func _configure_particles() -> void:
	...
	_fine_process_material.set_shader_parameter(&"emission_depth_min_m", underwater_particles_spawn_near_distance_m)
	_fine_process_material.set_shader_parameter(&"emission_depth_max_m", underwater_particles_spawn_far_distance_m)
	_fine_process_material.set_shader_parameter(&"emission_width_near_m", underwater_particles_spawn_near_width_m)
	_fine_process_material.set_shader_parameter(&"emission_width_far_m", underwater_particles_spawn_far_width_m)
	_fine_process_material.set_shader_parameter(&"emission_height_near_m", underwater_particles_spawn_near_height_m)
	_fine_process_material.set_shader_parameter(&"emission_height_far_m", underwater_particles_spawn_far_height_m)
	_fine_process_material.set_shader_parameter(&"camera_position_world", global_position)
	_fine_process_material.set_shader_parameter(&"max_particle_distance_m", underwater_particles_cull_distance_m)
	_fine_process_material.set_shader_parameter(&"layer_seed", 1.0)
	_fine_process_material.set_shader_parameter(&"drift_speed", underwater_particles_drift_speed)
	_fine_process_material.set_shader_parameter(&"velocity_influence", underwater_particles_velocity_influence)
	_fine_process_material.set_shader_parameter(&"max_flow_speed", 0.45)
	_fine_process_material.set_shader_parameter(&"current_velocity", Vector3(0.008, 0.001, -0.006))
	_fine_process_material.set_shader_parameter(&"camera_velocity", Vector3.ZERO)
	...
	_near_process_material.set_shader_parameter(&"emission_depth_min_m", underwater_particles_spawn_near_distance_m)
	_near_process_material.set_shader_parameter(&"emission_depth_max_m", underwater_particles_spawn_far_distance_m)
	_near_process_material.set_shader_parameter(&"emission_width_near_m", underwater_particles_spawn_near_width_m)
	_near_process_material.set_shader_parameter(&"emission_width_far_m", underwater_particles_spawn_far_width_m)
	_near_process_material.set_shader_parameter(&"emission_height_near_m", underwater_particles_spawn_near_height_m)
	_near_process_material.set_shader_parameter(&"emission_height_far_m", underwater_particles_spawn_far_height_m)
	_near_process_material.set_shader_parameter(&"camera_position_world", global_position)
	_near_process_material.set_shader_parameter(&"max_particle_distance_m", underwater_particles_cull_distance_m)
	_near_process_material.set_shader_parameter(&"layer_seed", 17.0)
	_near_process_material.set_shader_parameter(&"drift_speed", underwater_particles_drift_speed)
	_near_process_material.set_shader_parameter(&"velocity_influence", underwater_particles_velocity_influence)
	_near_process_material.set_shader_parameter(&"max_flow_speed", 0.80)
	_near_process_material.set_shader_parameter(&"current_velocity", Vector3(0.014, 0.003, -0.010))
	_near_process_material.set_shader_parameter(&"camera_velocity", Vector3.ZERO)
	...
	_fine_render_material.set_shader_parameter(&"near_fade_start_m", 0.15)
	_fine_render_material.set_shader_parameter(&"near_fully_visible_m", 0.70)
	...
	_near_render_material.set_shader_parameter(&"near_fade_start_m", 0.15)
	_near_render_material.set_shader_parameter(&"near_fully_visible_m", 0.70)
