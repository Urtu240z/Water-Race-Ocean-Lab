...
@export_range(0.0, 10.0, 0.1, "suffix: s") var underwater_particles_lifetime_fade_seconds := 2.0
...
func _make_process_material(layer_seed: float, size_min: float, size_max: float, current: Vector3, size_scale: float) -> ShaderMaterial:
	...
	material.set_shader_parameter(&"lifetime_fade_seconds", underwater_particles_lifetime_fade_seconds)
	material.set_shader_parameter(&"particle_lifetime_seconds", PARTICLE_LIFETIME_S)
	return material
