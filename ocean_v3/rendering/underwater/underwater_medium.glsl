...
bool sunrays_stable_light_coord(vec3 world_position, vec3 light_into_water, out vec2 stable_light_coord) {
	// This fixed light-space basis provides camera-independent shaft identity.
	...
	v = normalize(v);
	stable_light_coord = vec2(dot(world_position, u), dot(world_position, v));
	return !any(isnan(stable_light_coord)) && !any(isinf(stable_light_coord));
}

bool sunrays_ribbon_view_coord(vec3 sample_world, vec3 camera_world, vec3 light_into_water,
		out vec2 ribbon_view_coord, out vec3 ribbon_transverse_axis) {
	// A camera-facing ribbon has normal R = cross(L, view_to_camera). Its plane
	// is span(L, cross(R, L)), so it contains the physical light direction L.
	ribbon_view_coord = vec2(0.0);
	ribbon_transverse_axis = vec3(0.0);
	if (!finite_vec3(light_into_water) || length(light_into_water) <= EPSILON) return false;
	vec3 l = normalize(light_into_water);
	vec3 view_to_camera = camera_world - sample_world;
	if (finite_vec3(view_to_camera) && length(view_to_camera) > EPSILON) view_to_camera = normalize(view_to_camera);
	else view_to_camera = vec3(0.0);
	vec3 r = cross(l, view_to_camera);
	if (!finite_vec3(r) || length(r) <= EPSILON) {
		vec3 fallback_reference = abs(l.y) < 0.98 ? vec3(0.0, 1.0, 0.0) : vec3(1.0, 0.0, 0.0);
		r = cross(l, fallback_reference);
	}
	if (!finite_vec3(r) || length(r) <= EPSILON) return false;
	r = normalize(r);
	vec3 t = cross(r, l);
	if (!finite_vec3(t) || length(t) <= EPSILON) return false;
	t = normalize(t);
	ribbon_transverse_axis = r;
	ribbon_view_coord = vec2(dot(sample_world, r), dot(sample_world, t));
	return !any(isnan(ribbon_view_coord)) && !any(isinf(ribbon_view_coord));
}
...
float sunrays_beam_field(vec2 ribbon_view_coord, float width_factor, bool exaggerated) {
	float scale = max(params.sunrays_pattern.x, 0.01);
	float broad_phase = ribbon_view_coord.x * (0.72 * scale) + sin(ribbon_view_coord.y * 0.13) * 0.24;
	float medium_phase = ribbon_view_coord.x * (2.17 * scale) + ribbon_view_coord.y * 0.18 + 1.37;
	float narrow_phase = ribbon_view_coord.x * (3.49 * scale) - ribbon_view_coord.y * 0.08 + 0.61;
	...
	float slow_intensity = 0.84 + 0.16 * (0.5 + 0.5 * sin(ribbon_view_coord.y * 0.09));
...
bool evaluate_sunray_world_slice(vec3 camera_world, vec3 view_ray_world,
		...
		out vec3 light_entry, out vec3 transmittance, out float pattern, out vec2 ribbon_view_coord,
		out vec3 ribbon_transverse_axis,
	...
	light_entry = vec3(0.0); transmittance = vec3(0.0); pattern = 0.0; ribbon_view_coord = vec2(0.0);
	ribbon_transverse_axis = vec3(0.0);
	...
	if (!sunrays_ribbon_view_coord(sample_point, camera_world, light_into_water, ribbon_view_coord, ribbon_transverse_axis)) return false;
	vec2 stable_light_coord = vec2(0.0);
	if (!sunrays_stable_light_coord(sample_point, light_into_water, stable_light_coord)) return false;
	shaft_field = sunrays_beam_field(ribbon_view_coord, wave_width, exaggerated);
...
void main() {
	...
	vec2 ribbon_view_coord_debug = vec2(0.0); vec3 ribbon_axis_debug = vec3(0.0); float world_slice_id_debug = 0.0;
		...
		if (view_valid) {
			...
			if (sunray_segment_m > EPSILON && light_valid) {
				...
				for (int slice_offset = 0; slice_offset < 4; slice_offset++) {
					...
					vec3 slice_point; vec3 slice_entry; vec3 slice_transmittance; float slice_pattern; vec2 slice_ribbon_view_coord;
					vec3 slice_ribbon_axis;
					...
					if (!evaluate_sunray_world_slice(params.camera.xyz, view_ray_world, interval_begin_m, interval_end_m,
							...
							slice_point, slice_entry, slice_transmittance, slice_pattern, slice_ribbon_view_coord, slice_ribbon_axis,
					...
					pattern_debug += slice_pattern * slice_length_m; ribbon_view_coord_debug += slice_ribbon_view_coord * slice_length_m;
					ribbon_axis_debug += slice_ribbon_axis * slice_length_m;
				...
				if (integrated_length_m > EPSILON) {
					...
					ribbon_view_coord_debug /= integrated_length_m;
					vec3 averaged_ribbon_axis = ribbon_axis_debug / integrated_length_m;
					ribbon_axis_debug = length(averaged_ribbon_axis) > EPSILON ? normalize(averaged_ribbon_axis) : vec3(0.0);
	...
	else if (debug_mode == 14) color.rgb = valid_tap_count > 0 ? vec3(0.5 + 0.5 * sin(ribbon_view_coord_debug.x * 0.72), 0.5 + 0.5 * sin(ribbon_view_coord_debug.y * 0.72), 1.0) : vec3(0.0);
	...
	else if (debug_mode == 37) color.rgb = valid_tap_count > 0 ? ribbon_axis_debug * 0.5 + 0.5 : vec3(0.0);
	// The ribbon longitudinal axis is L itself, never its mirrored direction.
	else if (debug_mode == 38) color.rgb = light_valid ? light_into_water * 0.5 + 0.5 : vec3(0.0);
	else if (debug_mode >= 39 && debug_mode <= 41) color.rgb = vec3(0.0);
