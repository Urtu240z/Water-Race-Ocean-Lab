...
bool sunrays_beam_coord(vec3 world_position, vec3 light_into_water, out vec2 beam_coord) {
	// U/V span the plane orthogonal to L, therefore dot(P + L*d, U/V) is invariant.
...
void main() {
	...
	else if (debug_mode == 14) color.rgb = valid_tap_count > 0 ? vec3(0.5 + 0.5 * sin(beam_coord_debug.x * 0.72), 0.5 + 0.5 * sin(beam_coord_debug.y * 0.72), 1.0) : vec3(0.0);
	else if (debug_mode == 15) color.rgb = vec3(depth_response);
	else if (debug_mode == 16 || debug_mode == 22 || debug_mode == 28 || debug_mode == 37 || debug_mode == 38) color.rgb = sunray_contribution;
