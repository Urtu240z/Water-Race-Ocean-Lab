...
// Phase 1 deliberately intersects a flat y = sea_level interface.  The ocean
// surface shader remains the authority for the displaced visual interface.
...
layout(set = 0, binding = 2, std140) uniform Params {
	...
	vec4 camera; // xyz, stable underwater state
...
float water_path_for_scene(vec3 scene_world, bool scene_valid) {
	vec3 camera_world = params.camera.xyz;
	float sea_level = params.medium.x;
	bool camera_below = camera_world.y < sea_level;
	if (!scene_valid) {
		if (!camera_below) {
			return 0.0;
		}
		// A direction is all the sky case needs; this never uses far-plane length.
		vec3 sky_world;
		vec2 uv = (vec2(gl_GlobalInvocationID.xy) + vec2(0.5)) / params.viewport.xy;
...
void main() {
	...
	float water_path_m = clamp(water_path_for_scene(scene_world, scene_valid), 0.0, params.medium.z);
	...
	} else if (debug_mode < 6 && water_path_m > EPSILON) {
