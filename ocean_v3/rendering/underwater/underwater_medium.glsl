...
// RG = octahedral macro normal, B = normalized positive view depth,
// A = valid coverage plus camera-side encoding (1 air, 0.75 water).
...
layout(set = 0, binding = 2, std140) uniform Params {
	...
	vec4 medium; // density, max distance, waterline feather, interface depth max
...
const int DEBUG_OPAQUE_DEPTH = 3;
const int DEBUG_INTERFACE_NORMAL = 4;
const int DEBUG_VIEWER_MEDIUM = 5;
const int DEBUG_PIXEL_MEDIUM = 6;
const int DEBUG_SNELL_K = 7;
const int DEBUG_TIR = 8;
const int DEBUG_WATERLINE = 9;
...
void main() {
	...
	bool interface_valid = interface_sample.a > 0.5
		&& !isnan(interface_sample.b) && !isinf(interface_sample.b);
	float interface_depth = interface_valid ? interface_sample.b * max(params.medium.w, 1.0) : 0.0;
	bool interface_camera_air = interface_sample.a > 0.875;
	...
	// CROSSING is classified per interface pixel.  The side bit is generated by
	// the displaced interface material from the actual camera position and local
	// macro normal; no viewport rectangle or whole-frame CPU boolean is involved.
	float pixel_medium = viewer_medium == 0 ? 0.0 : viewer_medium == 1 ? 1.0
		: (interface_valid && !interface_camera_air ? 1.0 : 0.0);
	...
	if (debug_mode == DEBUG_INTERFACE_DEPTH) {
		store_debug(pixel, vec3(clamp(interface_sample.b, 0.0, 1.0))); return;
	}
	if (debug_mode == DEBUG_OPAQUE_DEPTH) {
		float d = texelFetch(resolved_depth, pixel, 0).r;
		store_debug(pixel, vec3(clamp(d, 0.0, 1.0))); return;
	}
	...
	float water_path_m;
	if (viewer_medium == 2 && interface_camera_air) {
