...
enum BreakingDebug { OFF, DEPTH, STEEPNESS, CRESTNESS, PREBREAK, OPEN_BREAK, DEPTH_BREAK, BREAK_ONSET, BREAK_STRENGTH, PARENT_HEIGHT, PARENT_CREST_SCALE, BAND_ATTRIBUTION, SEGMENT_SPAN, SEGMENT_ASYMMETRY, SEGMENT_COHERENCE }
...
const _BREAKING_DEBUG_NAMES := ["OFF", "DEPTH", "STEEPNESS", "CRESTNESS", "PREBREAK", "OPEN_BREAK", "DEPTH_BREAK", "BREAK_ONSET", "BREAK_STRENGTH", "PARENT_HEIGHT", "PARENT_CREST_SCALE", "BAND_ATTRIBUTION", "SEGMENT_SPAN", "SEGMENT_ASYMMETRY", "SEGMENT_COHERENCE"]
...
func _unhandled_input(event: InputEvent) -> void:
	...
	match event.keycode:
		...
		KEY_Y:
			_ocean.toggle_breaker_takeover_mask()
	_update_status()
...
func _breaker_text() -> String:
	...
	for index in summary["anchors"].size():
		...
		if state_name == "ACTIVE":
			...
			lines.append("slot %d DETECT wave=%d s=%+.1f score=%.2f p=%.2f roll=%.2f" % [index, int(t.get("wave", 0)), float(t.get("candidate_s", 0.0)), float(t.get("score", 0.0)), float(t.get("probability", 0.0)), float(t.get("roll", 0.0))])
