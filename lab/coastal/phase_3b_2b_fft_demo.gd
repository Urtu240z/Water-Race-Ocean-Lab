...
func _breaker_text() -> String:
	...
	return "Breakers (K): %s | debug (N): %s | %s | %s | %s | %s | slots %d/%d\n%s" % [enabled_text, _ocean.breaker_debug_name(), slot_text, stage_text, dir_text, mask_text, summary["slots"], summary["max_slots"], body]
