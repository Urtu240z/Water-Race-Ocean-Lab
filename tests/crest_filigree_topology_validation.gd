...
func _validate_solver_enable_policy() -> void:
	...
	for case_data in [
		...
		var topology_required: bool = case_data[3]
		var mid_history_required: bool = case_data[4]
		_check((surface_visible or filigree_visible) == topology_required, "%s technical topology requirement" % label)
		_check(surface_visible == mid_history_required, "%s MID history requirement" % label)
		surface_solver.set_settings(topology_required, 0.0, 1.0, 30.0)
		history_solver.set_settings(mid_history_required, 30.0, 0.16, 1.1, 0.10, 0.24)
		_check(surface_solver.get("_enabled") == topology_required, "%s Surface Foam solver state" % label)
		_check(history_solver.get("_enabled") == mid_history_required, "%s MID history solver state" % label)
...
func _validate_shader_contract() -> void:
	...
	_check(module.contains("var _surface_foam_mid_history_required := true"), "module stores independent MID history requirement")
	...
	_check(module.contains("_surface_foam_mid_history_required,\n\t\t_surface_foam_update_hz"), "MID history receives its own requirement")
	_check(module.contains("_surface_foam_mid_history_required = enabled"), "MID history follows Surface Foam visibility only")
